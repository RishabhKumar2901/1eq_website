...
const PurchasedWords = () => {
  ...
  const purchasedWords = words?.items?.filter(isPurchased) || [];

  return (
    <div className="flex flex-col justify-center items-center relative">
      <Navbar onRefreshClick={() => dispatch(fetchData("words"))} />

      <div className="flex flex-col justify-center items-center w-full max-w-4xl mt-4 px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 w-full relative">
          {words?.loading ? (
            <div className="flex w-full flex-wrap justify-center items-center fixed inset-0">
              <div className="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
            </div>
          ) : (
            purchasedWords.length > 0 &&
            purchasedWords.map((item, index) => (
              <div
                key={index}
        ...
        {!words?.loading && purchasedWords.length == 0 && (
          <div className="flex justify-center items-center w-full mt-40 text-2xl font-bold">
